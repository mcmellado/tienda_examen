...
<body>
    ...
    if (isset($login, $password, $password_repeat)) {
        ...
        if (preg_match('/[a-z]/', $password) !== 1) {
            $error['password'][] = 'La contraseña tiene que tener al menos 1 minúscula';
        }

        if (preg_match('/[A-Z]/', $password) !== 1) {
            $error['password'][] = 'La contraseña tiene que tener al menos 1 mayúscula';
        }

        if (preg_match('/[[:digit:]]/', $password) !== 1) {
            $error['password'][] = 'La contraseña tiene que tener al menos 1 dígito';
        }

        if (preg_match('/[[:punct:]]/', $password) !== 1) {
            $error['password'][] = 'La contraseña tiene que tener al menos 1 signo de puntuación';
        }


        foreach($error as $resultado_registro) {
        ...
        <div class="mx-72">
            <form action="" method="POST">
                <div class="mb-6">
                    ...
                    <?php foreach($error['login'] as $resultado): ?>
                ...
                <div class="mb-6">
                    ...
                    <?php foreach($error['password'] as $resultado): ?>
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-bold">¡Error!</span> <?= $resultado ?></p>
                    <?php endforeach ?>
                </div>
                <div class="mb-6">
                    ...
                    <?php foreach($error['password_repeat'] as $resultado): ?>
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-bold">¡Error!</span> <?= $resultado ?></p>
                    <?php endforeach ?>
                </div>
